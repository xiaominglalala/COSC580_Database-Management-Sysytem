...
def parse_select(sql):
    agg_operations=['max','min','count','avg','sum']
    sql=sql.lower()
    sql=re.sub(r';','',sql)
    select_lists = sqlparse.format(sql, reindent=True, indent_tabs=True, comma_first=True).split('\n')
    ...
    agg_regex=r'max[(]|min[(]|sum[(]|count[(]|avg[(]'
    columns = [re.match(agg_regex,i)[0][:-1]+' '+i.split('(')[-1].split(')')[0].strip() if re.match(agg_regex,i) else i for i in columns]
    columns_check=[i.split()[-1] for i in columns]
    ...
    if len(select_columns)!=0:
        ...
        for i in select_columns:
            ...
            if i.split()[0] not in columns_check:
                if columns==['*']:
                    pass
                else:
